...
IPlugVSTPlugin::IPlugVSTPlugin(IPlugInstanceInfo instanceInfo,
                   int nParams,
...
void IPlugVSTPlugin::BeginInformHostOfParamChange(int idx)
{
  mHostCallback(&mAEffect, audioMasterBeginEdit, idx, 0, 0, 0.0f);
}

void IPlugVSTPlugin::InformHostOfParamChange(int idx, double normalizedValue)
{
  mHostCallback(&mAEffect, audioMasterAutomate, idx, 0, 0, (float) normalizedValue);
}

void IPlugVSTPlugin::EndInformHostOfParamChange(int idx)
{
  mHostCallback(&mAEffect, audioMasterEndEdit, idx, 0, 0, 0.0f);
}

void IPlugVSTPlugin::InformHostOfProgramChange()
{
...
int IPlugVSTPlugin::GetSamplePos()
{
...
double IPlugVSTPlugin::GetTempo()
{
  if (mHostCallback)
  {
...
void IPlugVSTPlugin::GetTimeSig(int* pNum, int* pDenom)
{
  *pNum = *pDenom = 0;
...
void IPlugVSTPlugin::GetTime(ITimeInfo* pTimeInfo)
{
...
void IPlugVSTPlugin::AttachGraphics(IGraphics* pGraphics)
{
  if (pGraphics)
  {
...
void IPlugVSTPlugin::ResizeGraphics(int w, int h)
{
...
bool IPlugVSTPlugin::SendMidiMsg(IMidiMsg* pMsg)
{
...
bool IPlugVSTPlugin::SendSysEx(ISysEx* pSysEx)
{ 
...
audioMasterCallback IPlugVSTPlugin::GetHostCallback()
{
  return mHostCallback;
}

void IPlugVSTPlugin::HostSpecificInit()
{
...
VstIntPtr VSTCALLBACK IPlugVSTPlugin::VSTDispatcher(AEffect *pEffect, VstInt32 opCode, VstInt32 idx, VstIntPtr value, void *ptr, float opt)
{
  // VSTDispatcher is an IPlugVSTPlugin class member, we can access anything in IPlugVSTPlugin from here.
  IPlugVSTPlugin* _this = (IPlugVSTPlugin*) pEffect->object;
  if (!_this)
  {
    return 0;
  }
...
void IPlugVSTPlugin::VSTPrepProcess(SAMPLETYPE** inputs, SAMPLETYPE** outputs, VstInt32 nFrames)
{
  if (DoesMIDI())
  {
...
// Deprecated.
void VSTCALLBACK IPlugVSTPlugin::VSTProcess(AEffect* pEffect, float** inputs, float** outputs, VstInt32 nFrames)
{
  TRACE_PROCESS;
  IPlugVSTPlugin* _this = (IPlugVSTPlugin*) pEffect->object;
...
void VSTCALLBACK IPlugVSTPlugin::VSTProcessReplacing(AEffect* pEffect, float** inputs, float** outputs, VstInt32 nFrames)
{
  TRACE_PROCESS;
  IPlugVSTPlugin* _this = (IPlugVSTPlugin*) pEffect->object;
  IMutexLock lock(_this);
...
void VSTCALLBACK IPlugVSTPlugin::VSTProcessDoubleReplacing(AEffect* pEffect, double** inputs, double** outputs, VstInt32 nFrames)
{
  TRACE_PROCESS;
  IPlugVSTPlugin* _this = (IPlugVSTPlugin*) pEffect->object;
  IMutexLock lock(_this);
...
float VSTCALLBACK IPlugVSTPlugin::VSTGetParameter(AEffect *pEffect, VstInt32 idx)
{
...
void VSTCALLBACK IPlugVSTPlugin::VSTSetParameter(AEffect *pEffect, VstInt32 idx, float value)
{
