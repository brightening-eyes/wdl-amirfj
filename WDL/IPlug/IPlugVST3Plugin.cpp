...
IPlugVST3Plugin::~IPlugVST3Plugin() {}
...
tresult PLUGIN_API IPlugVST3Plugin::terminate ()
{
  TRACE;
...
tresult PLUGIN_API IPlugVST3Plugin::setupProcessing (ProcessSetup& newSetup)
{
  TRACE;
...
//tresult PLUGIN_API IPlugVST3Plugin::setState(IBStream* state)
//{
//  TRACE;
...
//}
//
//tresult PLUGIN_API IPlugVST3Plugin::getState(IBStream* state)
//{
//  TRACE;
//  WDL_MutexLock lock(&mMutex);
//
...
//tresult PLUGIN_API IPlugVST3Plugin::setComponentState(IBStream *state)
//{
//  TRACE;
//  WDL_MutexLock lock(&mMutex);
//
...
tresult PLUGIN_API IPlugVST3Plugin::canProcessSampleSize(int32 symbolicSampleSize)
{
...
Steinberg::uint32 PLUGIN_API IPlugVST3Plugin::getLatencySamples () 
{ 
  return mLatency;
} 
...
IPlugView* PLUGIN_API IPlugVST3Plugin::createView (const char* name)
{
...
tresult PLUGIN_API IPlugVST3Plugin::setParamNormalized(ParamID tag, ParamValue value)
{
  IParam* param = GetParam(tag);
...
tresult PLUGIN_API IPlugVST3Plugin::getParamStringByValue(ParamID tag, ParamValue valueNormalized, String128 string)
{
  IParam* param = GetParam(tag);
...
tresult PLUGIN_API IPlugVST3Plugin::getParamValueByString (ParamID tag, TChar* string, ParamValue& valueNormalized)
{
  return SingleComponentEffect::getParamValueByString (tag, string, valueNormalized);
}

void IPlugVST3Plugin::addDependentView(IPlugVST3View* view)
{
  viewsArray.add(view);
}

void IPlugVST3Plugin::removeDependentView(IPlugVST3View* view)
{
...
tresult IPlugVST3Plugin::beginEdit(ParamID tag)
{
  if (componentHandler)
...
tresult IPlugVST3Plugin::performEdit(ParamID tag, ParamValue valueNormalized)
{
  if (componentHandler)
...
tresult IPlugVST3Plugin::endEdit(ParamID tag)
{
  if (componentHandler)
...
AudioBus* IPlugVST3Plugin::getAudioInput (int32 index)
{
  AudioBus* bus = FCast<AudioBus> (audioInputs.at(index));
  return bus;
}

AudioBus* IPlugVST3Plugin::getAudioOutput (int32 index)
{
...
SpeakerArrangement IPlugVST3Plugin::getSpeakerArrForChans(int32 chans)
{
  switch (chans)
  {
    case 1:
...
int32 PLUGIN_API IPlugVST3Plugin::getUnitCount()
{
  TRACE;
...
tresult PLUGIN_API IPlugVST3Plugin::getUnitInfo(int32 unitIndex, UnitInfo& info)
{
  TRACE;
...
int32 PLUGIN_API IPlugVST3Plugin::getProgramListCount()
{
#ifdef VST3_PRESET_LIST
  return (NPresets() > 0);
#else
...
tresult PLUGIN_API IPlugVST3Plugin::getProgramListInfo(int32 listIndex, ProgramListInfo& info /*out*/)
{
  if (listIndex == 0)
  {
...
void IPlugVST3Plugin::BeginInformHostOfParamChange(int idx)
{
...
void IPlugVST3Plugin::EndInformHostOfParamChange(int idx)
{
  Trace(TRACELOC, "%d", idx);
  endEdit(idx);
}

void IPlugVST3Plugin::GetTime(ITimeInfo* pTimeInfo)
{
...
double IPlugVST3Plugin::GetTempo()
{
  return mProcessContext.tempo;
}

void IPlugVST3Plugin::GetTimeSig(int* pNum, int* pDenom)
{
  *pNum = mProcessContext.timeSigNumerator;
  *pDenom = mProcessContext.timeSigDenominator;
}

int IPlugVST3Plugin::GetSamplePos()
{
...
void IPlugVST3Plugin::SetLatency(int latency)
{
...
//void IPlugVST3Plugin::DumpFactoryPresets(const char* path, int a, int b, int c, int d)
//{
//  FUID pluginGuid;
...
IPlugVST3View::IPlugVST3View(IPlugVST3Plugin* pPlug)
  : mPlug(pPlug)
